...
async function getRoles() {
  const [rows] = await db.query(
    "SELECT id, name FROM roles WHERE excluido < 1 ORDER BY id",
  );
...
async function getPermissions() {
  const [rows] = await db.query(
    "SELECT id, name FROM permissions WHERE excluido < 1 ORDER BY id",
  );
...
async function getMatrix() {
  ...
  for (const row of rows) {
    ...
    matrix[row.role][row.permission] =
      row.access === "full" ? true : row.access === "view" ? "view" : false;
  }
...
async function saveMatrix(matrix) {
  ...
  try {
    ...
    const [rolesRows] = await conn.query(
      "SELECT id, name FROM roles WHERE excluido < 1",
    );
    const [permRows] = await conn.query(
      "SELECT id, name FROM permissions WHERE excluido < 1",
    );
    ...
    if (inserts.length > 0) {
      // build parameterized VALUES list for Postgres and use ON CONFLICT
      const placeholders = inserts.map(() => "(?, ?, ?)").join(",");
      const flat = inserts.flat();
      const sql = `INSERT INTO role_permissions (role_id, permission_id, access) VALUES ${placeholders} ON CONFLICT (role_id, permission_id) DO UPDATE SET access = EXCLUDED.access`;
